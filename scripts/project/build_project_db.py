...
DEFAULT_PROJECT_DB = ROOT / "data/duckdb/subs_project.duckdb"
DEFAULT_SOURCE_DB = ROOT / "data/duckdb/subs.duckdb"


def tbl_exists(con: duckdb.DuckDBPyConnection, q: str) -> bool:
    try:
...
def domain_stats(con: duckdb.DuckDBPyConnection, qname: str, domain_col: str = "domain"):
    df = con.execute(f"""
        SELECT
          COUNT(*) AS total,
...
def build_gold(con: duckdb.DuckDBPyConnection):
...
def build_ptbrvarid_views(con: duckdb.DuckDBPyConnection):
    src_ptbr = None
    for cand in ["src.main.ptbrvarid", "src.ptbrvarid", "src.main.PtBrVId", "src.PtBrVId"]:
        if tbl_exists(con, cand):
            src_ptbr = cand
            break
    if not src_ptbr:
    ...
    print("\n--- DOMAIN STATS src ---")
    domain_stats(con, src_ptbr, "domain")
...
def build_frmt(con: duckdb.DuckDBPyConnection):
    ...
    df_dev  = ds_dev.to_pandas().rename(columns={"br": "text_pt_br", "pt": "text_pt_pt"})
    df_test = ds_test.to_pandas().rename(columns={"br": "text_pt_br", "pt": "text_pt_pt"})
...
def build_opus_source(con: duckdb.DuckDBPyConnection):
    ...
    chosen = None
    for q in candidates:
        if tbl_exists(con, q) and has_cols(q):
            chosen = q
            break
...
def build_train_test_views(con: duckdb.DuckDBPyConnection):
    ...
    print("\n=== Unified view sizes ===")
    print("train_data rows:", con.execute("SELECT COUNT(*) FROM train_data").fetchone()[0])
    print("test_data rows :", con.execute("SELECT COUNT(*) FROM test_data").fetchone()[0])
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Build subs_project.duckdb views (PtBrVarId + FRMT + OPUS + Gold).")
    ap.add_argument("--project-db", default=str(DEFAULT_PROJECT_DB), help="Project DB path")
    ap.add_argument("--source-db", default=str(DEFAULT_SOURCE_DB), help="Source DB path")
    args = ap.parse_args()

    project_db = pathlib.Path(args.project_db).expanduser().resolve()
